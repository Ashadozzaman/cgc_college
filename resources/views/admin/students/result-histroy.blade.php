            ...
            <div class="card">
                <div class="card-body">
                    <div class="user-title row">
                        <h4 class="col-md-10"><strong class="text-denger">{{$student->full_name}}</strong> Result History</h4>
                        <a href="{{ route('get.students') }}" class="btn btn-primary btn-sm col-md-2"><i
                                class="fa fa-arrow-left"></i> Back</a>
                    </div>
                    <br>
                    @include('_message')
                    @foreach ($exam_wises as $item)
                    <h4 class="col-md-10"> {{$item['exam']}}</h4>
                    <div class="subject">
                            ...
                            <tbody>
                                <tr>
                                    @foreach ($item['main_subjects_result'] as $m_val)
                                        ...
                                        @endphp
                                        <td style="background: {{$background}}">{{($m_val['result'] == "F")?$m_val['result']:number_format($m_val['result'], 2)}}</td>
                                    @endforeach
                                    <td>{{($item['main_total_gpa'] == "F")?$item['main_total_gpa']:number_format($item['main_total_gpa'], 2)}}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    <div class="subject">
                        <table class="kt-table-result- table table-striped table-bordered dt-responsive nowrap"
                            ...
                            <thead>
                                <tr>
                                    @foreach($item['subjects'] as $val)
                                        <th title="{{$val->main_subject->name}}">{{strtoupper(substr($val->main_subject->name, 0, 3))}}<br>{{$val->subject_code}}</th>
                                    @endforeach
                                    {{-- <th>GPA</th> --}}
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    <td colspan="{{count($item['totalMarks'])}}" class="text-center">Total Mark Each Subjects</td>
                                </tr>
                                <tr>
