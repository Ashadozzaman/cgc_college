...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
            <h4 class="page-title mb-0 font-size-18">Mark Setup List</h4>
        ...
        <div class="card">
            <div class="card-body">
                <div class="user-title row">
                    <h4 class="col-md-10">Mark Setup List</h4>
                    <a href="{{route('mark_setup.create')}}" class="btn btn-primary btn-sm col-md-2"><i class="fa fa-plus"></i>Mark Setup</a>
                </div>
                <br>
                @include('_message')
                <table id="kt-table" class="kt-table table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        @foreach($mark_setups as $key=> $item)
                        <tr>
                            <td>{{$key+1}}</td>
                            ...
                            <td><a class="btn btn-success btn-sm" href="{{route('result.create',$item->id)}}">Result</a></td>
                            <td>
                                <div style="display: flex;">
                                    <a href="{{route('mark_setup.edit',$item->id)}}" class="fa fa-edit"></a>
                                    <form action="{{route('mark_setup.destroy',$item->id)}}" method="POST">
                                        @csrf
                                        @method('delete')
