...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
            <h4 class="page-title mb-0 font-size-18">Important Links List</h4>
        ...
        <div class="card">
            <div class="card-body">
                <div class="user-title row">
                    <h4 class="col-md-10">Important Links List</h4>
                    <a href="{{route('important_link.create')}}" class="btn btn-primary btn-sm col-md-2"><i class="fa fa-plus"></i>Add important_link</a>
                </div>
                <br>
                @include('_message')
                <table id="kt-table" class="kt-table table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        @foreach($important_links as $key=> $important_link)
                        <tr>
                            <td>{{$key+1}}</td>
                            <td>{{$important_link->title}}</td>
                            <td>{{$important_link->link}}</td>
                            <td>{{$important_link->order_by}}</td>
                            <td>
                            ...
                            <td>
                                <div style="display: flex;">
                                    <a href="{{route('important_link.edit',$important_link->id)}}" class="fa fa-edit"></a>
                                    <form action="{{route('important_link.destroy',$important_link->id)}}" method="POST">
                                        @csrf
                                        @method('delete')
